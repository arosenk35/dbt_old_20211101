  ...
  config({
	...
	distinct on (rxno,status_date)
        ...
        when (s.no_of_refill>s.fill_number and status_date::date>=s.rx_expire_date::date) then s.status_date
        ...
        when (s.no_of_refill>s.fill_number and status_date::date>=s.rx_expire_date::date) then 'Expired'
        when round(date_part('day',status_date::timestamp-rs.first_fill_date)/nullif(s.days_supply,0))-s.fill_number >3
        and  rs.last_refill_date < now()-interval '2 month'
		then 'Lost'
        else 'Open'
        ...
        when (s.no_of_refill>s.fill_number and s.status_date::date>=s.rx_expire_date::date) then 3
        when round(date_part('day',status_date::timestamp-rs.first_fill_date)/nullif(s.days_supply,0))-s.fill_number >3
        and  rs.last_refill_date < now()-interval '2 month'
		then 4
        else 2
